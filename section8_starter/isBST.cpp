 ...
 * binary searcg tree.
 */
bool isBSTRec(Node* root, int max, int min) {
    if (root == nullptr) {
        return true;
    }
    ...
    return isBSTRec(root->left, root->data, min) && isBSTRec(root->right, max, root->data);
}

bool isBST(Node* root) {
    return isBSTRec(root, std::numeric_limits<int>::max(), std::numeric_limits<int>::min());
}
