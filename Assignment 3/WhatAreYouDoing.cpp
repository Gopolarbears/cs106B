 ...
 */
string stringnize(const Vector<string>& tokens) {
    string res;
...
void allEmphasesOfHelper(const Vector<string>& tokens, Set<string>& allEmphases, int index) {
    ...
    newTokens[index] = toUpperCase(tokens[index]);
    allEmphases.add(stringnize(newTokens));
    for (int i = index + 1; i < tokens.size(); i++) {
        allEmphasesOfHelper(newTokens, allEmphases, i);
    }
}

Set<string> allEmphasesOf(const string& sentence) {
    Set<string> allEmphases;
    string newSentence = toLowerCase(sentence);
    Vector<string> tokens = tokenize(newSentence);
    allEmphases.add(stringnize(tokens));
    for (int i = 0; i < tokens.size(); i++) {
        allEmphasesOfHelper(tokens, allEmphases, i);
    }
    return allEmphases;
}
