 ...
 */
Set<Shift> highestValueScheduleForHelper(const Set<Shift>& shiftsLeft, const Set<Shift>& shiftsSelected, int& value, int timeLeft) {
    if (shiftsLeft.size() == 0) {
        return shiftsSelected;
    }

    Shift curShift = shiftsLeft.first();
    int value1 = 0;
    Set<Shift> res1 = highestValueScheduleForHelper(shiftsLeft - curShift, shiftsSelected, value1, timeLeft);
    value = value1;

    int value2 = 0;
    if (timeLeft < lengthOf(curShift)) {
        return res1;
    }
    for (Shift shift : shiftsSelected) {
        if (overlapsWith(shift, curShift)) {
            return res1;
        }
    }
    Set<Shift> res2 = highestValueScheduleForHelper(shiftsLeft - curShift, shiftsSelected + curShift, value2, timeLeft - lengthOf(curShift));
    value2 += valueOf(curShift);
    if (value1 > value2) {
        value = value1;
        return res1;
    } else {
...
Set<Shift> highestValueScheduleFor(const Set<Shift>& shifts, int maxHours) {
    if (maxHours < 0) {
        error("maxHours should be none-negative");
    }
    int value = 0;
    return highestValueScheduleForHelper(shifts, {}, value, maxHours);
}
