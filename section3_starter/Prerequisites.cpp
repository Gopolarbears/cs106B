 ...
 */
void listLegalOrderingsOfHelper(const Map<string, Set<string>> &prereqs, const Vector<string> &tasksPerformed, const Set<string> &tasksRemaining) {
    if (tasksRemaining.isEmpty()) {
        cout << endl << tasksPerformed;
        return;
    }
    ...
    for (string takeOneTask : tasksRemaining) {
        if (prereqs[takeOneTask].isSubsetOf(tasksDone)) {
            Vector<string> newTasksPerformed = tasksPerformed;
            newTasksPerformed.add(takeOneTask);
            listLegalOrderingsOfHelper(prereqs, newTasksPerformed, tasksRemaining - takeOneTask);
        }
    }
}
void listLegalOrderingsOf(const Map<string, Set<string>>& prereqs) {
    Vector<string> tasksPerformed;
    Set<string> tasksRemaining;
    for (string task : prereqs.keys()) {
        tasksRemaining.add(task);
    }
    listLegalOrderingsOfHelper(prereqs, {}, tasksRemaining);
}
