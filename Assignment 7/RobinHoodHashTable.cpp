...
bool RobinHoodHashTable::insert(const string& elem) {
    ...
    string target = elem;
    int index = hashFn(target);
    int distance = 0;
    for (int i = 0; i < capacity; i++) {
        if (elems[index].distance >= 0 && elems[index].value == target) {
            return false;
        }
        if (elems[index].distance == EMPTY_SLOT) {
            elems[index].distance = distance;
            elems[index].value = target;
            numElems++;
            return true;
        }
        if (elems[index].distance >= 0 && distance > elems[index].distance) {
            string temp = elems[index].value;
            int tempDitance = elems[index].distance;
            elems[index].value = target;
            elems[index].distance = distance;
            target = temp;
...
bool RobinHoodHashTable::contains(const string& elem) const {
    ...
    int distance = 0;
    for (int i = 0; i < capacity; i++) {
        if (elems[index].distance == EMPTY_SLOT || elems[index].distance < distance) {
            return false;
        }
        if (elems[index].distance >= 0 && elems[index].value == elem) {
            return true;
        }
        index = ((index + 1) % capacity);
        distance++;
...
bool RobinHoodHashTable::remove(const string& elem) {
    ...
    index = ((index + 1) % capacity);
    for (int i = 0; i < capacity; i++) {
        string tempElem = elems[index].value;
        int homeIndex = hashFn(tempElem);
        if (homeIndex == index || elems[index].distance == EMPTY_SLOT) {
            break;
        }
        int prevIndex = index > 0 ? index - 1 : capacity - 1;
        if (elems[prevIndex].distance == EMPTY_SLOT) {
            elems[prevIndex].value = tempElem;
            elems[prevIndex].distance = prevIndex >= homeIndex ? prevIndex - homeIndex : prevIndex + capacity - homeIndex;
            elems[index].distance = EMPTY_SLOT;
        }
        index = ((index + 1) % capacity);
    }
