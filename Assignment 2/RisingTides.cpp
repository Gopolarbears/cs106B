...
bool isLegalGridLocation(const GridLocation& source, int row, int col) {
    if (source.row < 0 || source.row >= row || source.col < 0 || source.col >= col) {
        return false;
    }
    return true;
}
void floodRegion(const Grid<double>& terrain, Grid<bool>& isUnderWater, const GridLocation& source, double height) {
    int row = terrain.numRows();
    int col = terrain.numCols();
    if (source.row < 0 || source.row >= row || source.col < 0 || source.col >= col) {
        return;
    }
    if (isUnderWater[source.row][source.col]) {
        return;
    }

    if (terrain[source.row][source.col] <= height) {
        isUnderWater[source.row][source.col] = true;
        if (isLegalGridLocation({source.row + 1, source.col}, row, col) && !isUnderWater[source.row + 1][source.col]) {
            floodRegion(terrain, isUnderWater, {source.row + 1, source.col}, height);
        }
        if (isLegalGridLocation({source.row - 1, source.col}, row, col) && !isUnderWater[source.row - 1][source.col]) {
            floodRegion(terrain, isUnderWater, {source.row - 1, source.col}, height);
        }
        if (isLegalGridLocation({source.row, source.col + 1}, row, col) && !isUnderWater[source.row][source.col + 1]) {
            floodRegion(terrain, isUnderWater, {source.row, source.col + 1}, height);
        }
        if (isLegalGridLocation({source.row, source.col - 1}, row, col) && !isUnderWater[source.row][source.col - 1]) {
            floodRegion(terrain, isUnderWater, {source.row, source.col - 1}, height);
        }
...
//                             double height) {
//     Grid<bool> isUnderWater(terrain.numRows(), terrain.numCols());
//     for (GridLocation source : sources) {
//         floodRegion(terrain, isUnderWater, source, height);
//     }
//     return isUnderWater;
// }
...
Grid<bool> floodedRegionsIn(const Grid<double>& terrain,
    ...
    int row = terrain.numRows();
    int col = terrain.numCols();
    Grid<bool> isUnderWater(row, col);
    Queue<GridLocation> floodedRegions;
    for (GridLocation source : sources) {
        floodedRegions.enqueue(source);
    }

    while (!floodedRegions.isEmpty()) {
        GridLocation source = floodedRegions.dequeue();
        if (!isLegalGridLocation(source, row, col) || terrain[source.row][source.col] > height || isUnderWater[source.row][source.col]) {
            continue;
        }
        isUnderWater[source.row][source.col] = true;
        floodedRegions.enqueue({source.row + 1, source.col});
        floodedRegions.enqueue({source.row - 1, source.col});
        floodedRegions.enqueue({source.row, source.col + 1});
        floodedRegions.enqueue({source.row, source.col - 1});
    }
