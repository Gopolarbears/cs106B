...
Map<string, double> kGramsIn(const string& str, int kGramLength) {
    ...
    for (int i = 0; i <= str.length() - kGramLength; i++) {
        ...
        kGrams[substr] = kGrams[substr] + 1;
        // if (kGrams.containsKey(substr)) {
        //     kGrams.put(substr, kGrams[substr] + 1);
        // } else {
        //     kGrams.put(substr, 1);
        // }
    }
...
bool hasNonZeroValues(const Map<string, double>& input) {
    if (input.isEmpty()) {
        return false;
    }
...
Map<string, double> normalize(const Map<string, double>& input) {
    if (!hasNonZeroValues(input)) {
        error("There should be non-zero values");
    }
    double sqrtSum = 0.0;
    for (double value : input.values()) {
        sqrtSum += pow(value, 2);
    }
    Map<string, double> newInput;
    for (string key : input.keys()) {
        newInput[key] = input[key] > 0 ? sqrt(input[key] * input[key] / sqrtSum) : -sqrt(input[key] * input[key] / sqrtSum);
    }
    return newInput;
}

Map<string, double> topKGramsIn(const Map<string, double>& source, int numToKeep) {
    PriorityQueue<string> kGramAndFrequence;
    for (string kGram : source.keys()) {
        kGramAndFrequence.enqueue(kGram, source.get(kGram));
    }
    for (int i = 0; i < source.size() - numToKeep; i++) {
        kGramAndFrequence.dequeue();
    }

    Map<string, double> result;
    while (!kGramAndFrequence.isEmpty()) {
        string key = kGramAndFrequence.dequeue();
...
double cosineSimilarityOf(const Map<string, double>& lhs, const Map<string, double>& rhs) {
    ...
    for (string kGram : lhs.keys()) {
        similarity += lhs.get(kGram) * rhs.get(kGram);
    }
    return similarity;
}
...
string guessLanguageOf(const Map<string, double>& textProfile,
    ...
    if (corpora.isEmpty()) {
        error("Corpor is empty");
    }
    double maxSimilairty = 0.0;
    string guessedLanguage;
    for (Corpus c : corpora) {
        double similarity = cosineSimilarityOf(textProfile, c.profile);
        if (similarity >= maxSimilairty) {
            maxSimilairty = similarity;
            guessedLanguage = c.name;
        }
    }
