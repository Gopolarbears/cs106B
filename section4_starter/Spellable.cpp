 ...
 */
bool isElementSpellableHelper(const string &text, const Set<string> &symbols, int start) {
    if (start >= text.size()) {
        return true;
    }
    ...
    bool oneAlpha = false;
    bool twoAlpha = false;
    string oneAlphaString = toUpperCase(text.substr(start, 1));
    string twoAlphaString = toUpperCase(text.substr(start, 1)) + text.substr(start + 1, 1);

    if (symbols.contains(oneAlphaString)) {
        oneAlpha = isElementSpellableHelper(text, symbols, start + 1);
    }
    if (symbols.contains(twoAlphaString)) {
        twoAlpha = isElementSpellableHelper(text, symbols, start + 2);
    }
    return oneAlpha || twoAlpha;
}

bool isElementSpellable(const string& text, const Set<string>& symbols) {
    return isElementSpellableHelper(text, symbols, 0);
}
