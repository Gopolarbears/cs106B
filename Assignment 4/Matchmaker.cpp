...
bool hasPerfectMatching(const Map<string, Set<string>>& possibleLinks, Set<Pair>& matching) {
    ...
    string memberA = possibleLinks.firstKey();
    if (possibleLinks.get(memberA).isEmpty()) {
        return false;
    }
    for (string memberB : possibleLinks.get(memberA)) {
        if (!possibleLinks.containsKey(memberB)) {
            continue;
        }
        Pair pair(memberA, memberB);
        matching.add(pair);
        Map<string, Set<string>> newPossibleLinks = possibleLinks;
        newPossibleLinks.remove(memberA);
        newPossibleLinks.remove(memberB);
        if (hasPerfectMatching(newPossibleLinks, matching)) {
            return true;
        }
...
Set<Pair> maximumWeightMatchingRec(const Map<string, Map<string, int>>& possibleLinks, Set<Pair>& matching, int& value) {
    if (possibleLinks.isEmpty()) {
        return matching;
    }
    string memberA = possibleLinks.firstKey();
    ...
    newPossibleLinks.remove(memberA);
    Set<Pair> curPairs = maximumWeightMatchingRec(newPossibleLinks, matching, cur);
    if (cur > max) {
        max = cur;
        maxPairs = curPairs;
    }
    ...
    for (string memberB : possibleLinks.get(memberA).keys()) {
        cur = 0;
        if (!possibleLinks.containsKey(memberB)) {
            continue;
        }
        Pair pair(memberA, memberB);
        matching.add(pair);
        newPossibleLinks = possibleLinks;
        newPossibleLinks.remove(memberA);
        newPossibleLinks.remove(memberB);
        curPairs = maximumWeightMatchingRec(newPossibleLinks, matching, cur);
        cur += possibleLinks.get(memberA).get(memberB);
        matching.remove(pair);
        if (cur > max) {
            max = cur;
            maxPairs = curPairs;
...
Set<Pair> maximumWeightMatching(const Map<string, Map<string, int>>& possibleLinks) {
    Set<Pair> matching;
    int value = 0;
    return maximumWeightMatchingRec(possibleLinks, matching, value);
}
