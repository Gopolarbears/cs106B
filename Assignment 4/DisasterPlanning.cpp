...
//                             Set<string>& leftCities,
//                             Set<string>& unSelectedCities) {
//     if (leftCities.isEmpty()) {
//         return true;
//     }
//     if (numLeftCities <= 0) {
//         return false;
...
//     for (string city : unSelectedCities) {
//         Set<string> newLeftCities = leftCities;
//         Set<string> newUnSelectedCities = unSelectedCities;
//         newLeftCities.remove(city);
//         newUnSelectedCities.remove(city);
//         for (string otherCity : leftCities) {
//             if (roadNetwork.get(city).contains(otherCity)) {
//                 newLeftCities.remove(otherCity);
//             }
//         }
//         if (canBeMadeDisasterReadyRec(roadNetwork, numLeftCities - 1, supplyLocations, newLeftCities, newUnSelectedCities)) {
...
//     return false;
// }
bool canBeMadeDisasterReadyHelper(const Map<string, Set<string>>& roadNetwork, int numLeftCities, Set<string>& supplyLocations, const Set<string>& leftCities) {
    if (leftCities.isEmpty()){
        return true;
    }
    if (numLeftCities <= 0) {
    ...
    string city1 = leftCities.first();
    Set<string> city1_neighbors = roadNetwork.get(city1);
    if (canBeMadeDisasterReadyHelper(roadNetwork, numLeftCities - 1, supplyLocations, leftCities - city1 - city1_neighbors)) {
        supplyLocations += city1;
        return true;
    } else {
        for (string city2 : city1_neighbors) {
            if (canBeMadeDisasterReadyHelper(roadNetwork, numLeftCities - 1, supplyLocations, leftCities - city2 - roadNetwork.get(city2))) {
...
bool canBeMadeDisasterReady(const Map<string, Set<string>>& roadNetwork,
                            ...
                            Set<string>& supplyLocations) {
    if (numCities < 0) {
        error("numCities should be positive");
    }
    Set<string> leftCities;
    for (string cityKey : roadNetwork.keys()) {
        leftCities.add(cityKey);
        for (string city : roadNetwork.get(cityKey)) {
            leftCities.add(city);
        }
    }
    Set<string> unSelectedCities = leftCities;
    return canBeMadeDisasterReadyHelper(roadNetwork, numCities, supplyLocations, leftCities);
}
