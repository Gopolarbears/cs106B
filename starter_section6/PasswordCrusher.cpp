...
breakWeakPasswords(const Map<string, int>& stolenPasswordFile,
    ...
    Map<int, string> weakHashs;
    for (string knownWeakPassword : knownWeakPasswords) {
        weakHashs[hashFn(knownWeakPassword)] = knownWeakPassword;
    }

    Map<string, string> res;
    for (string username : stolenPasswordFile.keys()) {
        if (weakHashs.containsKey(stolenPasswordFile[username])) {
            res[username] = stolenPasswordFile[username];
        }
    }
    return res;
}
