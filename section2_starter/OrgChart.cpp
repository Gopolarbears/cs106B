...
bool areAtSameCompany(const string& p1,
    ...
    if (!bosses.containsKey(p1) && !bosses.containsKey(p2)) {
        return false;
    }
    return areAtSameCompany(bosses.containsKey(p1) ? bosses.get(p1) : p1, bosses.containsKey(p2) ? bosses.get(p2) : p2, bosses);
}
