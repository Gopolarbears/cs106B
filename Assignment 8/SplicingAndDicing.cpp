...
Nucleotide* toStrand(const string& str) {
    ...
    Nucleotide* first = new Nucleotide{str[0], nullptr, nullptr};
    Nucleotide* prev = first;
    for (int i = 1; i < str.size(); i++) {
        Nucleotide* cur = new Nucleotide{str[i], nullptr, prev};
        prev->next = cur;
        prev = cur;
    }
    return first;
...
Nucleotide* findFirst(Nucleotide* dna, Nucleotide* target) {
    ...
    while(dna != nullptr) {
        if (dna->value == target->value) {
            ...
            Nucleotide* curTarget = target->next;
            while(curDna != nullptr && curTarget != nullptr && curDna->value == curTarget->value) {
                curDna = curDna->next;
