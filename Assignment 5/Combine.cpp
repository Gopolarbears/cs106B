...
Vector<DataPoint> combine(const Vector<Vector<DataPoint>>& sequences) {
    ...
    Vector<DataPoint> subSequence1 = combine(sequences.subList(0, half));
    Vector<DataPoint> subSequence2 = combine(sequences.subList(half));

    Vector<DataPoint> combinedSequence;
    int index1 = 0;
    int index2 = 0;
    while (index1 < subSequence1.size() && index2 < subSequence2.size()) {
        if (subSequence1[index1].weight <= subSequence2[index2].weight) {
            combinedSequence.add(subSequence1[index1++]);
        } else {
            combinedSequence.add(subSequence2[index2++]);
        }
    }
    if (index1 == subSequence1.size()) {
        while (index2 < subSequence2.size()) {
            combinedSequence.add(subSequence2[index2++]);
        }
    } else {
        while (index1 < subSequence1.size()) {
            combinedSequence.add(subSequence1[index1++]);
        }
    }
