...
EncodingTreeNode* huffmanTreeFor(const string& str) {
    ...
    if (frequence.size() <= 1) {
        error("Can't huffmanTreeFor too little characters");
    }
    PriorityQueue<EncodingTreeNode*> pq;
    for (char ch : frequence.keys()) {
        EncodingTreeNode* temp = new EncodingTreeNode{ch, nullptr, nullptr};
        pq.enqueue(temp, frequence[ch]);
    }

    while (pq.size() > 1) {
        int weight0 = pq.peekPriority();
        ...
        temp->zero = zero;
        temp->one = one;
        pq.enqueue(temp, weight0 + weight1);
    }
    return pq.dequeue();
}
...
void encodeTree(EncodingTreeNode* tree, Queue<Bit>& bits, Queue<char>& leaves) {
    Stack<EncodingTreeNode*> nodes;
    nodes.push(tree);
    while (!nodes.isEmpty()) {
        EncodingTreeNode* cur = nodes.pop();
        if (isLeaf(cur)) {
            bits.enqueue(0);
            leaves.enqueue(cur->ch);
        } else {
            bits.enqueue(1);
            if (cur->one != nullptr) {
                nodes.push(cur->one);
