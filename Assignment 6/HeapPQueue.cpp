...
void HeapPQueue::swimUp() {
    int child = logicalSize;
    int parent = logicalSize / 2;
    while (parent > 0 && elems[child].weight < elems[parent].weight) {
        swap(elems[child], elems[parent]);
        child = parent;
        parent = parent / 2;
    }
}
...
void HeapPQueue::swimDown() {
    int parent = 1;
    int leftChild = parent * 2;
    int rightChild = parent * 2 + 1;
    while(leftChild <= logicalSize && (elems[parent].weight > elems[leftChild].weight || elems[parent].weight > elems[rightChild].weight)) {
        if (leftChild > logicalSize && rightChild > logicalSize) {
            break;
        }
        if (rightChild > logicalSize) {
            if (elems[parent].weight > elems[leftChild].weight) {
                swap(elems[parent], elems[leftChild]);
            }
            break;
        }
        if (elems[leftChild].weight <= elems[rightChild].weight) {
            swap(elems[parent], elems[leftChild]);
            parent = leftChild;
        } else {
            swap(elems[parent], elems[rightChild]);
            parent = rightChild;
        }
        leftChild = parent * 2;
...
DataPoint HeapPQueue::dequeue() {
    ...
    DataPoint res = elems[logicalSize];
    logicalSize--;
    swimDown();
    return res;
}
