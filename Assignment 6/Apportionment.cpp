...
Map<string, int> apportion(const Map<string, int>& populations, int numSeats) {
    if (numSeats < populations.size()) {
        error("NumSeats should be greater than number of states");
    }
    Map<string, int> res;
    HeapPQueue pq;
    for (string city : populations.keys()) {
        pq.enqueue({city, -populations.get(city) / sqrt(2)});
        res[city] = 1;
        numSeats--;
    }
    while(numSeats > 0) {
        string nextCity = pq.dequeue().name;
        res[nextCity]++;
        pq.enqueue({nextCity, -populations.get(nextCity) / (sqrt(res[nextCity] * (res[nextCity] + 1)))});
        numSeats--;
    }
    return res;
}
